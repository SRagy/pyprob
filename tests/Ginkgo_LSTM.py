...
simulatorginkgo.load_inference_network('Old_LSTM_Network_20')

for i in range(1,5):
    simulatorginkgo.train(iterations=20, importance_sample_size=5000)
    simulatorginkgo.save_inference_network(f'Old_LSTM_Network_{(i+1)*20}')
    ...
    with open(f'LSTM_posterior_{(i+1)*20}', 'wb') as f:
