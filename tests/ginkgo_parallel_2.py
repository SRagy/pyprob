...
file_folder = 'network_parallel_24'

if __name__ == '__main__':
    #torch.cuda.empty_cache()
    #mp.set_start_method('spawn', force=True)
    with open(f'{file_folder}/network_stats', 'a') as f:
        f.write('weight_truncation = True\n')
        f.write('latent_params = False\n')
        #f.write('round 1 num_traces = 2**14, iss = 2**15\n')
        f.write('rate = [3,3], num_traces = 2^13, importance_sample_size = 2^13, batch_size = 128\n')
    for i in range(1,161):
        # if i==1:
        #     run_string = 'simulatorginkgo.train(iterations = 1, num_traces=2**14, importance_sample_size=2**15, batch_size=128, num_workers=11)'
        # else:
        #     run_string = 'simulatorginkgo.train(iterations = 1, num_traces=2**13, importance_sample_size=2**13, batch_size=128, num_workers=11)'
        # output_file = f'{file_folder}/cprofile_output_{i}'
        # cProfile.runctx(run_string, globals(), locals(), filename=output_file)
        simulatorginkgo.train(iterations=1, num_traces=2 ** 13, importance_sample_size=2 ** 13, batch_size=128)
        simulatorginkgo.save_inference_network(f'{file_folder}/LSTM_Network_{i}')
        posterior = simulatorginkgo._dis_traces(num_traces=2000, inference_engine=InferenceEngine.DISTILLING_IMPORTANCE_SAMPLING, observe={'dummy':1})
        epsilon = simulatorginkgo.epsilon
        ess_pre = simulatorginkgo.ess_pre
        ess_post = simulatorginkgo.ess_post
        with open(f'{file_folder}/network_stats', 'a') as f:
            f.write(f'iteration = {i}, epsilon = {epsilon}, ESS ={(ess_pre, ess_post)}\n')
        with open(f'{file_folder}/LSTM_posterior_{i}', 'wb') as f:
